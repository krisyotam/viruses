...
function wipe_data() {
    if (confirm("Wipe all data?") == false) {
        return;
    }
    console.log("Wiping phone data");
    $("#id_spinner").show();
    var request = $.ajax({
        beforeSend: function (xhr, settings) {
            xhr.setRequestHeader("X-CSRFToken", csrftoken);
    ...
    request.done(function (data) {
        $("#id_spinner").hide();
        $("#id_wipe_choice").html('<div class="alert alert-danger">Wiped</div>');
        if (data.success) {
            addAlert(data.success, "alert-success");
        }
        if (data.info) {
