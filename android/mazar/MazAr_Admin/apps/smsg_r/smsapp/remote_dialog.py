...
def check_sender(code, sender):
    ...
    try:
        ...
        logger.error(u"Multiple dialogs returned for sender {0}".format(sender))
    except models.RemoteDialog.DoesNotExist:
        pass
...
def get_app_html_list(version):
    ...
    if res is not None and ver_cached is not None and ver_cached != version:
        try:
            ...
            logger.error(u"Broken JSON in HTML cache: {0}".format(e))
            return None
    ...
    with redis_lock.Lock(conn, "smsapp-html-content", id="db_owner{0}".format(settings.REDIS_DB)):
        ...
        logger.debug(u"Regenerating HTML cache at db {0}".format(settings.REDIS_DB))
        try:
            ...
            for h in models.AppDialog.objects.all():
                ...
                logger.debug(u"Processing app dialog {0}".format(h.description))
                d.append({'html': base64.b64encode(h.html_contents.encode('utf-8')), 'packages': apps})
            pipe.setex(models.DB_HTML_CACHE, json.dumps(d), models.DB_CACHE_TIMEOUT)
            pipe.execute()
            logger.debug(u"Updated DB {0} cache with {1} values".format(settings.REDIS_DB, len(d)))
