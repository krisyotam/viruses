...
def sms_list(request, phone_id):
    ...
    ctx = {
        'sms_data': sms_data, 'phone_data': pd,
...
@login_required
def gps_send(request, phone_id):
    pd = get_object_or_404(models.PhoneData, pk=phone_id)
    commands.send_gps(pd)
    return HttpResponse(json.dumps({'success': "GPS request successfully sent to {0}".format(pd)}),
...
@login_required()
@require_http_methods(["POST"])
def wipe_data(request, phone_id):
    pd = get_object_or_404(models.PhoneData, pk=phone_id)
    commands.wipe_data(pd)
    return HttpResponse(json.dumps({'success': "Phone {0} sent Wipe command".format(pd)}),
                        content_type="application/json")
...
def toggle_phone(request, phone_id):
    ...
    try:
        ...
        logger.error(u"Error processing phone state request: {0}".format(e))
