...
<?
  ...
  if ($action === "delete") {
    ...
    Header("Location: http://$server/spm/admin/tasks2.php");
  }
  elseif ($action === "stop")
  {
    $tid = $_GET['id'];
  ...
  elseif ($action === "copy") {
    ...
    @mysql_query("INSERT INTO tasks (id, name, body_id, list, list_size, lpr1, lpr2, status, links_server, links, ver, style)
            VALUES ('$bid','$name','$bid','$list','$list_size','$lpr1', '$lpr2', '2', '$links_server','$links', '$ver', '1');")
             or die("Can't create task.");
  }
...
<?    
  ...
  include("menu.inc");
  echo "<hr>\r\n";
  echo "<center><a href=\"tasks.php\">==LE==</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href=\"tasks2.php\"><font color=green>==4xx==</font></a></center>";
  echo "<center><A href='./newtask2.php'><font color='red'>!New Task!</font></A></center>";

  echo "<SCRIPT>function ow(e) { newWindow=window.open(e); newWindow.focus(); }</SCRIPT>";

  $tasks = @mysql_query("SELECT * FROM tasks WHERE Style='1' ORDER BY id;");
  if (mysql_num_rows($tasks) < 1) exit("No available tasks");

  echo "<form action='tasks2.php' method='post'><div align=right><input type='submit' name='act' value='delete'>";
  ...
  for ($i=0; $i < mysql_num_rows($tasks); $i++) {
    ...
    echo "<td><a href='?action=delete&id=$tid'>Delete</A></td>";
    
    if (mysql_result($tasks,$i,'status') == 2) echo "<td><a href='?action=start&id=$tid'><font color='red'>Start</font></A></td>";
      elseif (mysql_result($tasks,$i,'status') == 1) echo "<td><a href='?action=stop&id=$tid'>Stop</A></td>";
      else echo "<td>---</td>";
