...
<?
    ...
    $sql = "SELECT * FROM robo ORDER BY `ver` ASC;";
    ...
    echo "<br>Good Online<br>";
    $tmparr = array(0);
    
    $sql = "SELECT * FROM robo WHERE (`time` > $need_date_u) AND (`smtp` = 'ok') ORDER BY `ver` ASC;";
    $result = @mysql_query($sql);    
    $num = @mysql_num_rows($result);    
    ...
    $sql = "SELECT * FROM robo WHERE `bl` = '1' ORDER BY `ver` ASC;";
    $result = @mysql_query($sql);    
    $num = @mysql_num_rows($result);    
