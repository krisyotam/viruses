...
typedef struct
{
  ...
  bool bCurrentFileNameAsAnsi;    //��������� ��� ����� � ANSI ���������.
  WCHAR strCurrentFile[MAX_PATH]; //������� ���� ��� ������.
  WCHAR strOutputFile[MAX_PATH];  //������� ���� ������.
  WCHAR strTempPath[MAX_PATH];    //��������� �����.
}FCIDATA;
#endif
...
static INT_PTR __FxIOpen(LPWSTR pszFile, int oflag, int pmode)
{
  ...
  if(h != INVALID_HANDLE_VALUE)
  {
    ...
    {
      p->fileName = Str::_CopyExW(pszFile, -1); //��� �������� �� ��������, �� ��������� ���.
      p->handle = h;
      p->oflags = oflag;
      return (INT_PTR)p;
    }
    ...
    if(p != NULL)
    {
      if(Fs::_pathCombine(p->strOutputFile, pstrPath, pstrFile))
      {
        ...
        if(k)
        {
          ...
          p->cab.iDisk          = 1;
          //p->cab.setID          = 0;
          Mem::_copy(p->cab.szCab, MSCAB_DLL, sizeof(MSCAB_DLL)); //�� ����� ������.
