...
$i = 0;
$output = str_replace('{WIDTH}', (COUNTRYLIST_WIDTH * 3).'px', THEME_LIST_BEGIN).
...
{
  $actionList = str_replace(array('{COLUMNS_COUNT}', '{TEXT}'), array(4, LNG_STATS_BOTNET_ACTIONS.THEME_STRING_SPACE.
...
str_replace(array('{COLUMNS_COUNT}', '{TEXT}'), array(4, LNG_STATS_BOTNET.THEME_STRING_SPACE.botnetsToListBox(CURRENT_BOTNET, '')), THEME_DIALOG_TITLE).
$actionList;
...
THEME_DIALOG_ROW_BEGIN. 
   str_replace('{COLUMNS_COUNT}', 1, THEME_DIALOG_ITEM_CHILD_BEGIN).
    listCountries(LNG_STATS_COLUMN_TOTALBOTS, '`rtime_last`>=\''.($config['botnet_timeout']).'\''.$commonQuery).
  THEME_DIALOG_ITEM_CHILD_END. 
 str_replace('{COLUMNS_COUNT}', 1, THEME_DIALOG_ITEM_CHILD_BEGIN).
