        ...
        $.getJSON(admurl+"gate/getBallance/"+botid+"/"+ver7+"/?callback=?", function(bal){
              ...
              var span = tr[i].getElementsByTagName('SPAN');
                for (var j = 0; j < bal.length; j++){
                   if (span[3].innerHTML==bal[j]['acc'] && span[1].innerHTML==top.window.frames.RT_IC_NVGT.document.getElementById('csn').innerHTML && span[5].innerHTML=='RUR' && span[7].innerHTML=='�/�'){
